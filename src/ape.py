...
class APE:
    def __init__(self, timesteps=16, use_decoder=False, encoding_size=512, multiple_feature_pred=False, **config):
    ...
    def train(self, states, actions, int_rewards,
        ...
        for epoch in range(self.config["n_epochs"]):
            for t in range(0, len(states), self.mini_batch_size):
                t_end = t+self.mini_batch_size

                state = torch.ByteTensor(states[t:t_end]).to(self.device)
                action = torch.ByteTensor(actions[t:t_end]).to(self.device)
                int_return = torch.Tensor(int_rets[t:t_end]).to(self.device)
                ext_return = torch.Tensor(ext_rets[t:t_end]).to(self.device)
                adv = torch.Tensor(advs[t:t_end]).to(self.device)
                old_log_prob = torch.Tensor(log_probs[t:t_end]).to(self.device)
                next_state = torch.Tensor(next_states[t:t_end]).to(self.device)
    ...
    def calculate_int_rewards(self, next_states, actions, batch=True):
        ...
        disc_loss = self.f1_loss(disc_preds[:, 0], mask[:, 0]) if self.multiple_feature_pred else torch.mean(self.f1_loss(disc_preds, mask), 1)
